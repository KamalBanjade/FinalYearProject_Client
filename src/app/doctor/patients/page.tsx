...
function PatientsContent() {
    ...
    const totalRecords = patients.reduce((sum, p: any) => sum + (p.sharedRecordsCount ?? p.SharedRecordsCount ?? 0), 0);
    const totalAppointments = patients.reduce((sum, p: any) => sum + (p.appointmentCount ?? p.AppointmentCount ?? 0), 0);
    ...
    return (
        <div className="max-w-7xl mx-auto px-6 md:px-8 pt-2 pb-12 space-y-6 animate-in fade-in duration-500 flex-1 w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard icon={FileText} label="Shared Records" value={totalRecords} colorClass="indigo" isLoading={isLoading} />
                <StatCard icon={Calendar} label="Appointments" value={totalAppointments} colorClass="emerald" isLoading={isLoading} />
            ...
            <ResponsiveTable
                ...
                columns={[
                    ...
                    },
                    {
                        ...
                        accessor: (p) => (
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">docs</span>
                            </div>
                        )
                    },
                    {
                        header: 'Appointments',
                        accessor: (p: any) => {
                            const count = p.appointmentCount ?? p.AppointmentCount ?? 0;
                            return (
                                <div className="flex items-center gap-2">
                                    <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">{count}</span>
                                    <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">visits</span>
                                </div>
                            );
                        }
                    },
                            ...
                            return (
                                <div className="flex items-center gap-2">
                                    <div className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                                        {isAppt ? <Calendar className="w-3.5 h-3.5 text-emerald-500" /> : <Clock className="w-3.5 h-3.5 text-slate-400" />}
                                    </div>
                                    <div>
                                        <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">{formatDate(lastDate)}</p>
                                        <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500">
                                            {isAppt ? 'Last Appointment' : 'Last Upload'}
                                        </p>
                                    </div>
                                </div>
                            );
                    ...
                    },
                    {
                        ...
                        accessor: (p) => (
                            <div className="flex flex-col items-end justify-center gap-2 p-1">
                                <button
                                    onClick={() => router.push(`/doctor/records/new?patientId=${p.id}&source=directory`)}
                                    className="group relative inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-blue-600 text-white transition-all duration-300 font-bold text-[10px] uppercase tracking-wider shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 hover:-translate-y-0.5 active:scale-[0.98] w-44 overflow-hidden"
                                >
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 hover:border-blue-500/40 transition-all duration-300 font-bold text-[10px] uppercase tracking-wider w-44 hover:shadow-md active:scale-[0.98]"
                                >
